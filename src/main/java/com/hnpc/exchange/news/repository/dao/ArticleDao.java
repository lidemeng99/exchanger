...
//    List<ArticleEntity> queryAllByColumnId(String columnId,);
    ...
    @Query("SELECT article FROM ArticleEntity article " +
            "where  article.article_id=?1 and article.last_updated>=TO_DATE(?2, 'YYYY-MM-DD HH24:MI:SS') ")
    List<ArticleEntity> queryAllChanged(String columnId,String lastModifyTime);
}
